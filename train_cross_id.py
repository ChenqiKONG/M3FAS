...
def parse_args():
    ...
    parser.add_argument('--val_model_epoch', default=1, type=int)
    ...
    parser.add_argument('--save_root', default='./Training_results/Cross_id_fusion/', type=str)
    parser.add_argument('--root_path', default='./', type=str)
    parser.add_argument('--model_name', default="s21", type=str) # Please change the phone device accordingly (note, s9, s21, xiaomi).

    parser.add_argument('--train_csv', default="./csv/Cross_5id_csv/s21_train.csv", type=str) # Please change the phone device accordingly (note, s9, s21, xiaomi.
    parser.add_argument('--val_csv', default="./csv/Cross_5id_csv/s21_val.csv", type=str) # Please change the phone device accordingly (note, s9, s21, xiaomi).
    parser.add_argument('--test_csv', default="./csv/Cross_5id_csv/s21_test.csv", type=str) # Please change the phone device accordingly (note, s9, s21, xiaomi).
...
def train(args, model):
    ...
    for epoch in range(args.warm_start_epoch, args.epochs):
        ...
        if (epoch+1) % args.val_model_epoch == 0:
            now_time = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            ...
            if not HTER_f > Best_HTER:
                #Best_HTER = HTER_f
                torch.save(model.state_dict(), res_folder_name + '/ckpt/' + 'fusion.pth')
